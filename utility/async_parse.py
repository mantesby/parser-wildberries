...
import aiohttp
import logging
from base_http import BaseHttpConifig
import dotenv
import os
import time


class AsyncWildBerriesParser(BaseHttpConifig):
    ...
    async def fetch_page(self, page, count):
    ...
    async def session_page(self, query, page):
        ...
        async with self.__session(cookies=self._cookie) as session:
            ...
            for page_json in pages_json["products"]:
                count += 1
                tasks.append(asyncio.create_task(self.fetch_page(page_json, count)))
